...
pub fn apply_prefab(mb: &mut MapBuilder, rng: &mut RandomNumberGenerator) {
    ...
    while placement.is_none() && attempts < 10 {
        let dimensions = bracket_geometry::prelude::Rect::with_size(
            rng.range(0, mb.map.dimensions.x - FORTRESS.1),
            rng.range(0, mb.map.dimensions.y - FORTRESS.2),
            FORTRESS.1,
            FORTRESS.2
        );
        ...
        for ty in placement.y .. placement.y + FORTRESS.2 {
            for tx in placement.x .. placement.x + FORTRESS.1 {
                ...
                match c {
                    ...
                    '-' => mb.map.tiles[idx] = Tile::new(mb.theme.tile_to_render(TileType::ThemeFloor)),
                    '#' => mb.map.tiles[idx] = Tile::new(mb.theme.tile_to_render(TileType::ThemeWall)),
